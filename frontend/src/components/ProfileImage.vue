  ...
  watch: {
    ...
    cachedUser: {
      handler(newCachedUser) {
        if (
          newCachedUser &&
          newCachedUser.profilePicture !== this.profilePicture
        ) {
        ...
        try {
          ...
          if (responseUser.status === 200 && responseUser.data) {
            ...
            if (responseUser.data.profilePicture) {
              ...
              if (
                savedAvatar &&
                savedAvatar !== responseUser.data.profilePicture
              ) {
              ...
              console.log(
                `ProfileImage: Using localStorage fallback for user ${this.id}:`,
                savedAvatar
              );
            }
          ...
          if (savedAvatar) {
            this.profilePicture = savedAvatar;
            console.log(
              `ProfileImage: API failed, using localStorage fallback for user ${this.id}:`,
              savedAvatar
            );
          } else {
